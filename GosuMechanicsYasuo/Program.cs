...
namespace GosuMechanicsYasuo
{
    class Program
    {
        ...
        public static Menu yasmenu, comboMenu, harassMenu, laneMenu, lasthitMenu, whitelistMenu, whitelistMenu1, jungleMenu, drawMenu, fleeMenu, miscMenu, wallMenu;
        ...
        public static void Game_OnGameLoad()
        {
            ...
            {
                Ignite = new Spell(slot, 600, DamageType.True);
            }
            ...
            comboMenu = yasmenu.AddSubMenu("Combo");
            ...
            comboMenu.Add("E3", new CheckBox("Mode: On = ToTarget / OFF = ToMouse", true));
            comboMenu.Add("Ignite", new CheckBox("Use Ignite", true));
            comboMenu.Add("comboItems", new CheckBox("Use Items", true));
            comboMenu.Add("myHP", new Slider("Use BOTRK if my hp <=", 70, 100, 1));
            comboMenu.Add("R", new CheckBox("Use Smart R", true));
            comboMenu.Add("R1", new Slider("when enemy HP <=", 5, 100, 1));
            comboMenu.Add("R2", new Slider("or when knockedUp enemy is >=", 2, 5, 1));
            comboMenu.Add("R3", new CheckBox("Use R instantly when an ally is in range", true));
            comboMenu.Add("R4", new CheckBox("Use Auto R", true));
            comboMenu.Add("R5", new Slider("when knockedUp enemy is >=", 3, 5, 1));
            comboMenu.Add("R6", new Slider("owhen <= enemy in range", 2, 5, 1));
            comboMenu.Add("R7", new Slider("when myHero HP is >=", 50, 101, 1));

            whitelistMenu = yasmenu.AddSubMenu("Ult Whitelist Settings");
            foreach (var hero in HeroManager.Enemies.Where(x => x.IsEnemy))
            {
                whitelistMenu.Add(hero.ChampionName, new CheckBox("Use Ulti if Target is: " + hero.ChampionName));
            }

            harassMenu = yasmenu.AddSubMenu("Harass Settings");
            harassMenu.Add("AutoQHarass", new KeyBind("Harass Toggle", false, KeyBind.BindTypes.PressToggle, "L".ToCharArray()[0]));
            harassMenu.Add("HarassQ", new CheckBox("Use Q12", true));
            harassMenu.Add("HarassQ3", new CheckBox("Use Q3", true));
            harassMenu.Add("HarassTower", new CheckBox("Harass UnderTower", false));


            lasthitMenu = yasmenu.AddSubMenu("LastHit Settings");
            lasthitMenu.Add("LastHitQ1", new CheckBox("Use Q12", true));
            lasthitMenu.Add("LastHitQ3", new CheckBox("Use Q3", true));
            lasthitMenu.Add("LastHitE", new CheckBox("Use E", true));


            laneMenu = yasmenu.AddSubMenu("LaneClear Settings");
            laneMenu.Add("LaneClearQ1", new CheckBox("Use Q12", true));
            laneMenu.Add("LaneClearQ3", new CheckBox("Use Q3", true));
            laneMenu.Add("LaneClearE", new CheckBox("Use E", true));
            laneMenu.Add("LaneClearQ3count", new Slider("when Q3 will hit minions >=", 2, 5, 1));
            laneMenu.Add("LaneClearItems", new CheckBox("Use Items", true));

            jungleMenu = yasmenu.AddSubMenu("JungleClear Settings");
            jungleMenu.Add("JungleClearQ12", new CheckBox("Use Q12", true));
            jungleMenu.Add("JungleClearQ3", new CheckBox("Use Q3", true));
            jungleMenu.Add("JungleClearE", new CheckBox("Use E", true));

            fleeMenu = yasmenu.AddSubMenu("Escape Settings");
            fleeMenu.Add("wall", new KeyBind("WallJump Escape", false, KeyBind.BindTypes.HoldActive, "N".ToCharArray()[0]));
            fleeMenu.Add("AutoQ1", new CheckBox("Use Q Stack while Dashing", true));
            fleeMenu.Add("AutoQToggle", new KeyBind("Auto Q Minion Toggle (Normal)", false, KeyBind.BindTypes.PressToggle, "K".ToCharArray()[0]));

            wallMenu = yasmenu.AddSubMenu("WindWall Settings");
            wallMenu.Add("smartW", new CheckBox("Use Auto WindWall", true));
            wallMenu.Add("smartWDanger", new Slider("if Spell DangerLevel >=", 3, 5, 1));
            wallMenu.Add("smartWDelay", new Slider("WindWall Humanizer (500 = Lowest Reaction Time)", 3000, 3000, 500));
            wallMenu.Add("smartEDogue", new CheckBox("Use E-Vade", true));
            wallMenu.Add("smartEDogueDanger", new Slider("if Spell DangerLevel >=", 1, 5, 1));
            wallMenu.Add("wwDanger", new CheckBox("Block only dangerous", false));

            whitelistMenu1 = yasmenu.AddSubMenu("Windwall on Combo Whitelist Settings");
            foreach (var hero in HeroManager.Enemies.Where(x => x.IsEnemy))
            {
                whitelistMenu1.Add(hero.ChampionName, new CheckBox("Use Put WallBehind if Enemy is: " + hero.ChampionName));

            }
            skillShotMenu = getSkilshotMenu();

            miscMenu = yasmenu.AddSubMenu("Misc Settings");
            miscMenu.Add("ETower", new CheckBox("Dont Jump turrets", true));
            miscMenu.Add("KS", new CheckBox("KillSteal", true));
            miscMenu.Add("IntAnt", new CheckBox("Use Q3 - AntiGapcloser/Interrupter", true));

            drawMenu = yasmenu.AddSubMenu("Draw Settings");
            drawMenu.Add("Disable", new CheckBox("Disable all draws", true));
            drawMenu.Add("DrawQ", new CheckBox("Draw Q12 Range", true));
            drawMenu.Add("DrawQ3", new CheckBox("Draw Q3 Range", true));
            ...
            drawMenu.Add("DrawSpots", new CheckBox("Draw WallJump Spots", true));
        ...
        private static void AntiGapcloser_OnEnemyGapcloser(ActiveGapcloser gapcloser)
        {
            ...
            if (Q3.IsReady() && Q3READY() && Getcheckboxvalue(miscMenu, "IntAnt"))
            {
                Q3.Cast(target);
            }
        }
        ...
        {
            if (sender != null && Q3.IsReady() && Q3READY() && sender.IsValidTarget(Q3.Range) && Getcheckboxvalue(miscMenu, "IntAnt"))
            {
                Q3.Cast(sender);
            }
        }
        ...
        private static void Game_OnUpdate(EventArgs args)
        {
            ...
            {
                if (Getcheckboxvalue(wallMenu, "smartW"))
                    useWSmart(mis);

                if (Getcheckboxvalue(wallMenu, "smartEDogue") && !W.IsReady() && !isSafePoint(ObjectManager.Player.Position.To2D(), true).IsSafe)
                    useEtoSafe(mis);
            }
            ...
            if (Getkeybindvalue(fleeMenu, "wall"))
            {
                Yasuo.WallJump();
                Yasuo.WallDash();
            }
            if (Getkeybindvalue(fleeMenu, "AutoQToggle") && Orbwalker.ActiveModesFlags == Orbwalker.ActiveModes.None && !Getkeybindvalue(fleeMenu, "wall"))
            {
                AutoQ();
            }
            ...
            if (!IsDashing && Getkeybindvalue(fleeMenu, "AutoQToggle") && Orbwalker.ActiveModesFlags != Orbwalker.ActiveModes.Combo)
            {
                var TsTarget = TargetSelector.GetTarget(1000, DamageType.Physical);

                if (TsTarget != null && TsTarget.CharData.BaseSkinName != "gangplankbarrel" && Getcheckboxvalue(harassMenu, "HarassTower") && Q3.IsReady() && Getcheckboxvalue(harassMenu, "HarassQ3") && !IsDashing && Q3READY() && Q3.IsInRange(TsTarget))
                {
                    CastQ3(TsTarget);
                }
                else if (TsTarget != null && !Getcheckboxvalue(harassMenu, "HarassTower") && TsTarget.CharData.BaseSkinName != "gangplankbarrel" && !UnderTower(myHero.ServerPosition.To2D()) && Q3.IsReady() && Getcheckboxvalue(harassMenu, "HarassQ3") && !IsDashing && Q3READY() && Q3.IsInRange(TsTarget))
                {
                    CastQ3(TsTarget);
                }
            }

            if (!IsDashing && Getkeybindvalue(harassMenu, "AutoQHarass") && Orbwalker.ActiveModesFlags != Orbwalker.ActiveModes.Combo)
            {
                var TsTarget = TargetSelector.GetTarget(475, DamageType.Physical);

                if (TsTarget != null && TsTarget.CharData.BaseSkinName != "gangplankbarrel" && Getcheckboxvalue(harassMenu, "HarassTower") && !Q3READY() && Q.IsReady() && Getcheckboxvalue(harassMenu, "HarassQ") && !IsDashing && Q.IsInRange(TsTarget))
                {
                    CastQ12(TsTarget);
                }
                else if (TsTarget != null && TsTarget.CharData.BaseSkinName != "gangplankbarrel" && !Getcheckboxvalue(harassMenu, "HarassTower") && !Q3READY() && Q.IsReady() && Getcheckboxvalue(harassMenu, "HarassQ") && !IsDashing && !UnderTower(myHero.ServerPosition.To2D()) && Q.IsInRange(TsTarget))
                {
                    CastQ12(TsTarget);
                }
            }
        ...
        public static void HarassLastHit()
        {
            foreach (Obj_AI_Base minion in MinionManager.GetMinions(myHero.ServerPosition, Q.Range, MinionTypes.All, MinionTeam.Enemy).OrderByDescending(m => m.Health))
            {
                ...
                if (!minion.IsDead && minion != null && Getcheckboxvalue(lasthitMenu, "LastHitQ1") && Q.IsReady() && minion.IsValidTarget(500) && !Q3READY() && Q.IsInRange(minion))
                {
        ...
        public static void Harass()
        {
            ...
            if (TsTarget != null && Getcheckboxvalue(harassMenu, "HarassTower"))
            {

                if (Q3.IsReady() && Getcheckboxvalue(harassMenu, "HarassQ3") && !IsDashing && Q3READY() && Q3.IsInRange(TsTarget))
                {
                    CastQ3(TsTarget);
                }
                else if (!Q3READY() && Q.IsReady() && Getcheckboxvalue(harassMenu, "HarassQ") && !IsDashing && Q.IsInRange(TsTarget))
                {
                    CastQ12(TsTarget);
                }
            }
            else if (TsTarget != null && !Getcheckboxvalue(harassMenu, "HarassTower"))
            {
                if (!UnderTower(myHero.ServerPosition.To2D()) && Q3.IsReady() && Getcheckboxvalue(harassMenu, "HarassQ3") && !IsDashing && Q3READY() && Q3.IsInRange(TsTarget))
                {
                    CastQ3(TsTarget);
                }
                if (!Q3READY() && Q.IsReady() && Getcheckboxvalue(harassMenu, "HarassQ") && !IsDashing && !UnderTower(myHero.ServerPosition.To2D()) && Q.IsInRange(TsTarget))
                {
                    CastQ12(TsTarget);
                }
            }
        ...
        public static void Flee()
        {
            ...
            if (E.IsReady())
            {
                var bestMinion = ObjectManager.Get<Obj_AI_Minion>().Where(x => x.IsValidTarget(E.Range)).Where(x => x.Distance(Game.CursorPos) < ObjectManager.Player.Distance(Game.CursorPos)).OrderByDescending(x => x.Distance(ObjectManager.Player)).FirstOrDefault();
        ...
        public static void AutoQFlee()
        {
            ...
            foreach (var minion in Qminion.Where(minion => minion.IsValidTarget(Q.Range) && !minion.IsDead))
            {
                ...
                if (!Q3READY() && Getcheckboxvalue(fleeMenu, "AutoQ1") && minion.IsValidTarget(Q.Range) && IsDashing)
                {
                    CastQ12(minion);
                }
            }
        ...
        public static void LastHit()
        {
            foreach (Obj_AI_Base minion in MinionManager.GetMinions(myHero.ServerPosition, Q3.Range, MinionTypes.All, MinionTeam.Enemy).OrderByDescending(m => m.Health))
            {
                ...
                if (!minion.IsDead && minion != null && Getcheckboxvalue(lasthitMenu, "LastHitQ1") && Q.IsReady() && minion.IsValidTarget(500) && !Q3READY() && Q.IsInRange(minion) && !IsDashing)
                {
                    var predHealth = HealthPrediction.GetHealthPrediction(minion, (int)(Program.myHero.Distance(minion.Position) * 1000 / 2000));
                    if (predHealth <= GetQDmg(minion))
                    {
                ...
                if (!minion.IsDead && minion != null && Getcheckboxvalue(lasthitMenu, "LastHitQ3") && Q.IsReady() && minion.IsValidTarget(1100) && Q3READY() && Q3.IsInRange(minion) && !IsDashing)
                {
                    var predHealth = HealthPrediction.GetHealthPrediction(minion, (int)(Program.myHero.Distance(minion.Position) * 1000 / 2000));
                    if (predHealth <= GetQDmg(minion))
                    {
                ...
                if (Getcheckboxvalue(lasthitMenu, "LastHitE") && E.IsReady() && minion.IsValidTarget(475))
                {
        ...
        public static void LaneClear()
        {
            ...
            foreach (var minion in Qminion.Where(minion => minion.IsValidTarget(Q3.Range)))
            {
                ...
                if (minion != null && Getcheckboxvalue(laneMenu, "LaneClearQ1") && Q.IsReady() && minion.IsValidTarget(500) && !Q3READY() && Q.IsInRange(minion))
                {
                    ...
                    {
                        List<Vector2> minionPs = GetCastMinionsPredictedPositions(Qminion, .025f, 50f, float.MaxValue, myHero.ServerPosition, 475f, false, SkillshotType.SkillshotLine); MinionManager.FarmLocation farm = Q.GetLineFarmLocation(minionPs);
                ...
                if (!minion.IsDead && minion != null && Getcheckboxvalue(laneMenu, "LaneClearQ3") && Q3.IsReady() && minion.IsValidTarget(1100) && Q3READY() && Q3.IsInRange(minion))
                {
                    var predHealth = HealthPrediction.LaneClearHealthPrediction(minion, (int)(Program.myHero.Distance(minion.Position) * 1000 / 2000));
                    if (predHealth <= GetQDmg(minion) && !IsDashing)
                    {
                    ...
                    else if (Q3READY() && Q3.IsInRange(minion) && !IsDashing)
                    {
                        ...
                        if (farm.MinionsHit >= Getslidervalue(laneMenu, "LaneClearQ3count"))
                        {
                            CastQ3(minion);
                        }
            ...
            foreach (var minion in allMinionsE.Where(x => x.IsValidTarget(E.Range) && CanCastE(x)))
            {
                ...
                if (Getcheckboxvalue(laneMenu, "LaneClearE") && E.IsReady() && minion.IsValidTarget(E.Range) && CanCastE(minion))
                {
                ...
                if (Getcheckboxvalue(laneMenu, "LaneClearItems"))
                {
                    UseItems(minion);
                }
            }
            ...
            foreach (var jungleMobs in jminions.Where(x => x.IsValidTarget(Q3.Range)))
            {
                ...
                if (Getcheckboxvalue(jungleMenu, "JungleClearE") && E.IsReady() && jungleMobs != null && jungleMobs.IsValidTarget(E.Range) && CanCastE(jungleMobs))
                {
                    E.Cast(jungleMobs);
                }
                if (jungleMobs != null && Getcheckboxvalue(jungleMenu, "JungleClearQ3") && Q3.IsReady() && jungleMobs.IsValidTarget(1000) && Q3READY() && Q3.IsInRange(jungleMobs))
                {
                    CastQ3(jungleMobs);
                }
                if (jungleMobs != null && Getcheckboxvalue(jungleMenu, "JungleClearQ12") && Q.IsReady() && jungleMobs.IsValidTarget(500) && !Q3READY() && Q.IsInRange(jungleMobs))
                {
                    CastQ12(jungleMobs);
                }
            }
        ...
        public static void UseItems(Obj_AI_Base unit)
        {
            if (Items.HasItem((int)ItemId.Blade_of_the_Ruined_King, myHero) && Items.CanUseItem((int)ItemId.Blade_of_the_Ruined_King)
               && Getcheckboxvalue(comboMenu, "comboItems") && HealthPercent <= Getslidervalue(comboMenu, "myHP"))
            {
        ...
        public static void AutoR()
        {
            ...
            var useR = Getcheckboxvalue(comboMenu, "R4");
            var autoREnemies = Getslidervalue(comboMenu, "R5");
            var MyHP = Getslidervalue(comboMenu, "R7");
            var enemyInRange = Getslidervalue(comboMenu, "R6");
        ...
        public static void Combo()
        {
            ...
            if (TsTarget != null && Getcheckboxvalue(comboMenu, "QC"))
            {
            ...
            if (Getcheckboxvalue(wallMenu, "smartW") && wallCasted && myHero.Distance(TsTarget.Position) < 300)
            {
                eBehindWall(TsTarget);
            }
            if (Getcheckboxvalue(comboMenu, "EC") && TsTarget != null)
            {
                ...
                if (E.IsReady() && TsTarget.Distance(myHero) >= (Getslidervalue(comboMenu, "E1")) && dmg >= TsTarget.Health && UnderTower(PosAfterE(TsTarget)) && CanCastE(TsTarget) && myHero.IsFacing(TsTarget))
                {
                    E.Cast(TsTarget);
                }
                else if(TsTarget.Distance(myHero) >= (Getslidervalue(comboMenu, "E1")) && dmg <= TsTarget.Health && CanCastE(TsTarget) && myHero.IsFacing(TsTarget))
                {
                    useENormal(TsTarget);
                }
                ...
                if (Getcheckboxvalue(comboMenu, "E3") && E.IsReady())
                {
                    var bestMinion = ObjectManager.Get<Obj_AI_Minion>().Where(x => x.IsValidTarget(E.Range)).Where(x => x.Distance(TsTarget) < myHero.Distance(TsTarget)).OrderByDescending(x => x.Distance(myHero)).FirstOrDefault();
                    ...
                    if (bestMinion != null && TsTarget != null && dmg2 >= TsTarget.Health && UnderTower(PosAfterE(bestMinion)) && myHero.IsFacing(bestMinion) && TsTarget.Distance(myHero) >= (Getslidervalue(comboMenu, "E2")) && CanCastE(bestMinion) && myHero.IsFacing(bestMinion))
                    {
                        E.Cast(bestMinion);
                    }
                    else if (bestMinion != null && TsTarget != null && dmg2 <= TsTarget.Health && myHero.IsFacing(bestMinion) && TsTarget.Distance(myHero) >= (Getslidervalue(comboMenu, "E2")) && CanCastE(bestMinion) && myHero.IsFacing(bestMinion))
                    {
                ...
                if (!Getcheckboxvalue(comboMenu, "E3") && E.IsReady())
                {
                       var bestMinion = ObjectManager.Get<Obj_AI_Minion>().Where(x => x.IsValidTarget(E.Range)).Where(x => x.Distance(Game.CursorPos) < ObjectManager.Player.Distance(Game.CursorPos)).OrderByDescending(x => x.Distance(myHero)).FirstOrDefault();
                    ...
                    if (bestMinion != null && TsTarget != null && dmg3 >= TsTarget.Health && UnderTower(PosAfterE(bestMinion)) && myHero.IsFacing(bestMinion) && TsTarget.Distance(myHero) >= (Getslidervalue(comboMenu, "E2")) && CanCastE(bestMinion) && myHero.IsFacing(bestMinion))
                    {
                        E.Cast(bestMinion);
                    }
                    else if (bestMinion != null && TsTarget != null && dmg3 <= TsTarget.Health && myHero.IsFacing(bestMinion) && TsTarget.Distance(myHero) >= (Getslidervalue(comboMenu, "E2")) && CanCastE(bestMinion) && myHero.IsFacing(bestMinion))
                    {
                        useENormal(bestMinion);
                    }
                }
            ...
            if (Program.R.IsReady() && Getcheckboxvalue(comboMenu, "R"))
            {
                ...
                foreach (AIHeroClient enemy in enemies)
                {
                    ...
                    {
                        var enemiesKnockedUp = ObjectManager.Get<AIHeroClient>().Where(x => x.IsValidTarget(R.Range)).Where(x => x.HasBuffOfType(BuffType.Knockup));

                        var enemiesKnocked = enemiesKnockedUp as IList<AIHeroClient> ?? enemiesKnockedUp.ToList();
                        if (enemy.IsValidTarget(Program.R.Range) && Program.CanCastDelayR(enemy) && enemiesKnocked.Count() >= (Getslidervalue(comboMenu, "R2")))
                        {
                            Program.R.Cast();
                        }
                    }
                    if (enemy.IsValidTarget(Program.R.Range))
                    {
                        ...
                        if (Program.IsKnockedUp(enemy) && Program.CanCastDelayR(enemy) && enemy.Health <= ((Getslidervalue(comboMenu, "R1") / 100 * enemy.MaxHealth) * 1.5f) && Getcheckboxvalue(whitelistMenu, enemy.ChampionName))
                        {
                            Program.R.Cast();
                        }
                        else if (Program.IsKnockedUp(enemy) && Program.CanCastDelayR(enemy) && enemy.Health >= ((Getslidervalue(comboMenu, "R1") / 100 * enemy.MaxHealth) * 1.5f) && (Getcheckboxvalue(comboMenu, "R3")))
                        {
                ...
                if (Getcheckboxvalue(comboMenu, "Ignite") && TsTarget.Health <= myHero.GetSummonerSpellDamage(TsTarget, Damage.SummonerSpell.Ignite))
                {
                    Ignite.Cast(TsTarget);
                }
                
                if (Getcheckboxvalue(comboMenu, "comboItems") && TsTarget != null && TsTarget.IsValidTarget())
                {
                    UseItems(TsTarget);
                }
            }
        ...
        public static void KillSteal()
        {
            ...
            {
                if (enemy.IsValidTarget(Q.Range) && Getcheckboxvalue(miscMenu, "KS"))
                {
        ...
        {
            return myHero.ServerPosition.LSExtend(target.ServerPosition, myHero.Distance(target) < 410 ? E.Range : myHero.Distance(target) + 65).To2D();
        }
        ...
        public static void putWallBehind(AIHeroClient target)
        {
            ...
            if (!target.IsMoving || myHero.Distance(dashPos) <= dist + 40)
                if (dist < 330 && dist > 100 && W.IsReady() && Getcheckboxvalue(whitelistMenu1, target.ChampionName))
                {
        ...
        public static IsSafeResult isSafePoint(Vector2 point, bool igonre = false)
        {
            ...
            bool safe = Orbwalker.ActiveModesFlags == Orbwalker.ActiveModes.Flee || point.To3D().GetEnemiesInRange(500).Count > myHero.HealthPercent % 65;
            if (!safe)
            {
                result.IsSafe = false;
                return result;
        ...
        public static void useEtoSafe(Skillshot skillShot)
        {
            ...
            if (closestTarg != null && closestTarg.CountEnemiesInRange(600) <= 2  && Getslidervalue(skillShotMenu, "DangerLevel" + skillShot.SpellData.MenuItemName) >= Getslidervalue(wallMenu, "smartEDogueDanger"))
        ...
        public static void useWSmart(Skillshot skillShot)
        {
            ...
            var WDelay = Getslidervalue(wallMenu, "smartWDelay");
            ...
            if (skillShot.IsAboutToHit(WDelay, myHero))
            {
                ...
                //if only dangerous
                if (Getcheckboxvalue(wallMenu, "wwDanger") && skillShotIsDangerous(sd.MenuItemName))
                {
                    myHero.Spellbook.CastSpell(SpellSlot.W, skillShot.Start.To3D(), skillShot.Start.To3D());
                }
                if (!Getcheckboxvalue(wallMenu, "wwDanger") && Getslidervalue(skillShotMenu, "DangerLevel" + sd.MenuItemName) >= Getslidervalue(wallMenu, "smartWDanger"))
                {
                    myHero.Spellbook.CastSpell(SpellSlot.W, skillShot.Start.To3D(), skillShot.Start.To3D());
                }
            }
        ...
        public static bool useENormal(Obj_AI_Base target)
        {
            ...
            if (!Getcheckboxvalue(miscMenu, "ETower"))
            {
            ...
            foreach (var hero in ObjectManager.Get<AIHeroClient>())
            {
                if (hero.Team != ObjectManager.Player.Team)
                {
                    foreach (var spell in SpellDatabase.Spells)
                    {
                        ...
                        {
                            var subMenu = MainMenu.AddMenu(spell.MenuItemName, spell.MenuItemName);


                            subMenu.Add("IsDangerous", new CheckBox("IsDangerous" + spell.MenuItemName, true));
                            subMenu.Add("DangerLevel" + spell.MenuItemName, new Slider("Danger Level", spell.DangerValue, 5, 1));
                            //subMenu.AddItem(new MenuItem("Draw" + spell.MenuItemName, "Draw").SetValue(true));
                            subMenu.Add("Enabled", new CheckBox("Enabled" + spell.MenuItemName, true));
        ...
        {
            if (Getcheckboxvalue(skillShotMenu, "IsDangerous" + Name) != null)
            {
                return Getcheckboxvalue(skillShotMenu, "IsDangerous" + Name);
            }
            return true;
        }
        public static bool EvadeSpellEnabled(string Name)
        {
            if (Getcheckboxvalue(skillShotMenu, "Enabled" + Name) != null)
            {
                return Getcheckboxvalue(skillShotMenu, "Enabled" + Name);
            }
            return true;
        }
